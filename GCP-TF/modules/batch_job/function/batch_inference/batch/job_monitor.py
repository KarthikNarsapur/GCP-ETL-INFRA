...
if not CROSS_ACCOUNT_ACCESS_KEY or not CROSS_ACCOUNT_SECRET_KEY:
    try:
        ...
        CROSS_ACCOUNT_ACCESS_KEY = secret_data.get('AWS_ACCESS_KEY_ID') or secret_data.get('access_key_id') or secret_data.get('CROSS_ACCOUNT_ACCESS_KEY')
        CROSS_ACCOUNT_SECRET_KEY = secret_data.get('AWS_SECRET_ACCESS_KEY') or secret_data.get('secret_access_key') or secret_data.get('CROSS_ACCOUNT_SECRET_KEY')
...
if CROSS_ACCOUNT_ACCESS_KEY and CROSS_ACCOUNT_SECRET_KEY:
    ...
    bedrock_client = aws_session.client('bedrock')
...
# Lambda client always uses default credentials (same account)
...
def monitor_batch_jobs():
    ...
    try:
        ...
        error_msg = str(e)
        print(f"  ❌ Error querying batch_job: {error_msg}")
        
        # Provide helpful error messages for common issues
        if "Cannot assign requested address" in error_msg or "Errno 99" in error_msg:
            print(f"  ⚠️  Network connectivity issue detected.")
            print(f"  ⚠️  If Lambda is in VPC, ensure:")
            print(f"      - NAT Gateway is configured for internet access")
            print(f"      - Security groups allow outbound HTTPS (443)")
            print(f"      - Route tables route 0.0.0.0/0 to NAT Gateway")
            print(f"      - DNS resolution is enabled in VPC")
        elif "Connection" in error_msg or "timeout" in error_msg.lower():
            print(f"  ⚠️  Connection timeout. Check API endpoint availability.")
